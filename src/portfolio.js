// src/portfolio.js — mode switching, rendering, hero sequence, theme
'use strict';

let _mode = 'fe', _isDark = true, _beR = false, _fsR = false;

// ── THEME ──
function initTheme() {
  const s = localStorage.getItem('lme-theme');
  _isDark = s ? s === 'dark' : window.matchMedia('(prefers-color-scheme:dark)').matches;
  _applyTheme();
}
function _applyTheme() {
  ...
  // Night Walker / Sun Summoner per feedback
  if (label) label.textContent = _isDark ? 'Night Walker' : 'Sun Summoner';
...
function setMode(m) {
  ...
  if (m === 'be' && !_beR) { _renderBE(); _beR = true; }
  if (m === 'fs' && !_fsR) { _renderFS(); _fsR = true; }
...
function _renderBE() {
  ...
  (window.PROJECTS || []).forEach((p, i) => {
    ...
    card.className = 'bc';
    card.innerHTML = `<div class="bc-num">PROJECT_${String(i+1).padStart(3,'0')}</div>
...
function _renderFS() {
  ...
  (window.PROJECTS || []).forEach(p => {
    ...
    let media = '';
    if (p.drive)      media = `<div class="ez" id="ez-${p.id}"><div class="ez-ph" onclick="ldrive('${p.id}','${p.drive}')"><button class="play-c">&#9654;</button><div class="ez-lbl">Click to play demo</div></div></div>`;
    else if (p.live)  media = `<div class="ez"><div class="ez-ph" onclick="window.open('${p.live}','_blank')"><button class="play-c">&#8599;</button><div class="ez-lbl">Open live demo</div></div></div>`;
    else              media = `<div class="ez"><div class="ez-ph" onclick="window.open('${p.repo}','_blank')"><button class="play-c">&#9645;</button><div class="ez-lbl">View repository</div></div></div>`;
    const card = document.createElement('div');
    card.className = 'fsc';
    card.innerHTML = `<div class="fsc-hd">
      ...
      </div>${media}<div class="fsc-ft">${links.join('')}</div>`;
    grid.appendChild(card);
  });
}
...
function ldrive(id, driveId) {
  ...
  ez.innerHTML = ''; ez.appendChild(iframe);
}
window.ldrive = ldrive;

// ── HERO TYPEWRITER ──
function _typer(id, txt, spd, cb) {
  const el = document.getElementById(id);
  if (!el) return cb && cb();
  el.textContent = ''; let i = 0;
  function go() { if (i < txt.length) { el.textContent += txt[i++]; setTimeout(go, spd); } else if (cb) cb(); }
  go();
}

function runHero() {
  const hw = document.getElementById('hw'), nm = document.getElementById('nm');
  if (hw) { hw.style.transform = 'translateY(20px)'; }
  if (nm) { nm.style.transform = 'translateY(20px)'; }
  setTimeout(() => {
    if (hw) { hw.style.transition = 'opacity .5s, transform .5s'; hw.style.opacity = '1'; hw.style.transform = 'none'; }
    _typer('hwTxt', 'Hello World', 60, () => {
      if (nm) { nm.style.transition = 'opacity .5s, transform .5s'; nm.style.opacity = '1'; nm.style.transform = 'none'; }
      setTimeout(() => _typer('nmTxt', 'LuckyME', 55, () => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => { if (window.initWhoami) window.initWhoami(); }, 500);
        }, 300);
      }), 300);
    });
  }, 300);
